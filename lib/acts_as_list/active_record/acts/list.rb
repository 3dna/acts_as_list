        ...
        private
          ...
          def shuffle_positions_on_intermediate_items(old_position, new_position, avoid_id = nil)
            ...
            if old_position < new_position
              ...
              acts_as_list_class
                .unscoped
                .where("#{scope_condition} AND #{position_column} > #{old_position} AND #{position_column} <= #{new_position}#{avoid_id_condition}")
                .update_all("#{position_column} = (#{position_column} - 1)")
            else
              ...
              acts_as_list_class
                .unscoped
                .where("#{scope_condition} AND #{position_column} >= #{new_position} AND #{position_column} < #{old_position}#{avoid_id_condition}")
                .update_all("#{position_column} = (#{position_column} + 1)")
            end
          end
